...
parser.add_argument("--dp_size", type=int, default=1, help="Data parallelism size for sglang (--dp-size N). When > 1, enables data parallelism")
parser.add_argument("--dp_router", action=argparse.BooleanOptionalAction, default=False, help="Use SMG router (sglang_router) instead of native DP for cache-aware routing")
...
{export_stuff}
python bench_p3.py --path_archive {path_archive} --path_save {path_save} --name_experience {name_experience} --n_generation {n_generation} --num_solutions {num_solutions} --seed {seed} --model_name_or_path {model_name_or_path} {extra}
"""
...
for model in list_model:
    ...
    if args.presence_penalty:
        extra += f" --presence_penalty {args.presence_penalty}"
    if args.fp8:
        extra += " --fp8"
    else:
        extra += " --no-fp8"
    ...
    job_name = f"gen_sol_ACES_P3_model-{model_id}"+"_"+args.name_experience + "_nsolution-"+str(args.num_solutions)
